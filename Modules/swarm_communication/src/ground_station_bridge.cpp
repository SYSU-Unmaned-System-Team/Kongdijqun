...
//     // drone state length
//     int ds_length = sizeof(int8_t)+3*3*sizeof(float);send_sock
...
void connect_to_ground_station(const char *ip, const int port, struct sockaddr_in &serv_addr)
{
    /* Connect */
    //  int send_sock = 0; 
    cout << "=========================" << endl; 
...
void drone_state_cb(const prometheus_msgs::DroneState::ConstPtr& msg,int& uav_id_)
{
    int len = encode_drone_state(msg,uav_id_);
    struct sockaddr_in serv_addr;
    connect_to_ground_station(tcp_ip.c_str(), TCP_PORT, serv_addr);
