...
void Global_Planner::init(ros::NodeHandle& nh)
{
    ...
    station_cmd_sub = nh.subscribe<prometheus_msgs::StationCommandCase3>("/case3/command_uav", 1, &Global_Planner::cmd_cb, this);
    // 【地面站交互】将检测结果发送至地面站（所有从机均使用该指令）
    case3_result_pub = nh.advertise<prometheus_msgs::Case3Result>("/case3/uav_result", 10);
    // 【地面站交互】发布提示消息
...
void Global_Planner::cmd_cb(const prometheus_msgs::StationCommandCase3ConstPtr& msg)
{
    station_cmd = *msg;

    if(station_cmd.Command_uav == prometheus_msgs::StationCommandCase3::Start)
    {
        station_ready = true;
        // 判断本机是否需要执行任务
        // if(station_cmd.Flag_id >0 )
        // {
        //     station_ready = true;
        // }else
        // {
        ...
        //     waypoints_[0][0] = station_cmd.goal_uav[0];
        //     waypoints_[0][1] = station_cmd.goal_uav[1];
        //     waypoints_[0][2] = fly_height_2D;
        // }
        // return;
    }else if(station_cmd.Command_uav == prometheus_msgs::StationCommandCase3::Return)
    {
        exec_state = EXEC_STATE::RETURN;
        message = "Get command from ground station: [ RETURN ]";
        pub_message(message_pub, prometheus_msgs::Message::NORMAL, node_name, message);
        return;
    }else if(station_cmd.Command_uav == prometheus_msgs::StationCommandCase3::Land)
    {
        exec_state = EXEC_STATE::LAND;
    ...
    if(station_cmd.detection_flag == prometheus_msgs::StationCommandCase3::detected && detected_by_myself == false)
    {
    ...
    }else
    {
        ...
        if(num_count_vision_get > 10)
        {
            ...
            && object_pos_enu(1)<detection_range_y(1) && object_pos_enu(1)>detection_range_y(0))
            {
                ...
                prometheus_msgs::Case3Result result;
                ...
                // 发布
                case3_result_pub.publish(result);
            }else
            {
